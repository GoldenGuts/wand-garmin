...
TARGET=${1:-prg}
[ $# -gt 0 ] && shift
if [ "$TARGET" = "store" ]; then HA_URL=""; WEBHOOK_ID=""; TARGET=iq; STORE=1; fi
...
case "$TARGET" in
  ...
  demo)
    ...
    ;;
  *)
    echo "usage: ./build.sh [prg|sim|iq|demo] [extra monkeyc args]"; rm -f resources/properties/properties.xml; exit 2
    ;;
esac
rm -f resources/properties/properties.xml   # never leave the webhook id in the tree
if [ -n "$STORE" ] && [ -n "$(cat webhook_id.txt 2>/dev/null)" ] && grep -q -a "$(cat webhook_id.txt)" build/Wand.iq; then
  echo "ERROR: the webhook id is inside build/Wand.iq"; rm -f build/Wand.iq; exit 1
fi
case "$TARGET" in
